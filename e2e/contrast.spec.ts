 ...
 *
 * The defect that motivated this port at dcyfr-io was in `app/global-error.tsx`:
 * a `bg-primary` button carrying no text token at all, inheriting
 * `text-foreground` from its own `<body>`. That is 1.00:1 in dark — not
 * low-contrast, absent. Note that this file is NOT reachable by route (it
 * renders only on an unhandled root error), so the gate below cannot cover
 * it; it is guarded by the sibling parity with dcyfr-app instead.
 *
